  ...
  HRESULT STDMETHODCALLTYPE DDrawInterface::CreateSurface(LPDDSURFACEDESC lpDDSurfaceDesc, LPDIRECTDRAWSURFACE *lplpDDSurface, IUnknown *pUnkOuter) {
    ...
    if (likely(lpDDSurfaceDesc->dwFlags & DDSD_CAPS)) {
      ...
      if (unlikely((lpDDSurfaceDesc->ddsCaps.dwCaps & DDSCAPS_OVERLAY)
        ...
        Logger::info("DDrawInterface::CreateSurface: Overlay support is disabled, skipping");
        return DDERR_UNSUPPORTED;
      }
  ...
  HRESULT STDMETHODCALLTYPE DDrawInterface::EnumSurfaces(DWORD dwFlags, LPDDSURFACEDESC lpDDSD, LPVOID lpContext, LPDDENUMSURFACESCALLBACK lpEnumSurfacesCallback) {
    ...
    for (auto& attachedSurface : attachedSurfaces) {
      if (likely(hr == DDENUMRET_OK)) {
        Com<IDirectDrawSurface> surface = attachedSurface.surface;
        Com<DDrawSurface> ddrawSurface;
        try {
          ddrawSurface = new DDrawSurface(nullptr, std::move(surface), this, nullptr, false);

          hr = lpEnumSurfacesCallback(ddrawSurface.ref(), &attachedSurface.desc, lpContext);
        } catch (const DxvkError& e) {
          Logger::warn("DDrawInterface::EnumSurfaces: Failed to create wrapped surface");
          Logger::warn(e.message());
        }
      } else if (likely(attachedSurface.surface != nullptr)) {
        ...
        attachedSurface.surface->Release();
      }
    }
