  ...
  */
  ...
  public:
    ...
    DDrawSurface* GetAttachedDepthStencil() {
      ...
      Com<IDirectDrawSurface> depthStencil;
      HRESULT hr = m_proxy->GetAttachedSurface(&caps, &depthStencil);
      if (unlikely(FAILED(hr)))
        return nullptr;

      try {
        m_depthStencil = new DDrawSurface(nullptr, std::move(depthStencil),
                                          m_commonIntf->GetDDInterface(), this, false);
      } catch (const DxvkError& e) {
        Logger::err("DDrawSurface::GetAttachedDepthStencil: Failed to create wrapped surface");
