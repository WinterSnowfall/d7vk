  ...
  */
  ...
  public:
    ...
    DDraw3Surface* GetAttachedDepthStencil() {
      ...
      Com<IDirectDrawSurface3> depthStencil;
      HRESULT hr = m_proxy->GetAttachedSurface(&caps, &depthStencil);
      if (unlikely(FAILED(hr)))
        return nullptr;

      try {
        m_depthStencil = new DDraw3Surface(nullptr, std::move(depthStencil), m_parent, this);
      } catch (const DxvkError& e) {
        Logger::err("DDraw3Surface::GetAttachedDepthStencil: Failed to create wrapped surface");
