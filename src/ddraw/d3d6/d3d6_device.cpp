...
namespace dxvk {
  ...
  HRESULT STDMETHODCALLTYPE D3D6Device::SetRenderTarget(IDirectDrawSurface4 *surface, DWORD flags) {
    ...
    HRESULT hr = rt6->GetCommonSurface()->ValidateRTUsage(m_commonD3DDevice->IsHALOrTNLHALDevice(), false, false);
    if (unlikely(FAILED(hr)))
      return hr;
        ...
        switch (dwRenderState) {
          ...
          case D3DTBLEND_MODULATE: {
            ...
            if (m_textures[0] != nullptr) {
              DDrawCommonSurface* commonSurface = m_textures[0]->GetCommonTexture()->GetCommonSurface();
